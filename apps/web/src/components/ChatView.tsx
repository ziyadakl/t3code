...
export default function ChatView(props: ChatViewProps) {
  ...
  const showScrollDebouncer = useRef(
    new Debouncer(
      () => {
        // Don't show the pill when there's nothing to scroll (content fits the
        // viewport) or we're already at the end. LegendList can leave a stale
        // isAtEnd=false on a non-overflowing list and never fire a corrective
        // scroll event, which would otherwise strand the pill visible-and-dead.
        const state = legendListRef.current?.getState?.();
        if (state && isEffectivelyAtEnd(state)) {
          return;
        }
  ...
  // Append highlighted transcript text into the composer draft (never replaces
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    // Hoist an inline `/command` in the USER'S TYPED PROMPT to the front so the
    // Agent SDK actually fires it (the SDK only treats a leading command as a
    // command). Scan ONLY the typed prompt, BEFORE appending terminal context —
    // otherwise a `/command`-shaped token inside pasted terminal output could be
    // hoisted and fire an unintended (possibly destructive) command. Hoisting
    // before the effort prefix keeps the command leading in the final text
    ...
    const messageTextForSend = appendTerminalContextsToPrompt(
      hoistLeadingSlashCommand(promptForSend, ctxSlashCommandNames),
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div ref={messagesWrapperRef} className="relative flex min-h-0 flex-1 flex-col">
            ...
            <SelectionQuoteToolbar
              boundaryRef={messagesWrapperRef}
              onQuote={quoteSelectionIntoComposer}
            />
          </div>
