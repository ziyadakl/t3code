...
/**
 * True when the composer holds exactly `/resume` (optionally with trailing
 * whitespace). t3 intercepts this as its own command — opening the resume
 * picker — instead of forwarding it to the provider, which has its own
 * unrelated `/resume`.
 */
