...
### The conflict-prone "hot files" (verified 2026-06-03; re-confirmed during the v0.0.27 adoption 2026-06-15 — `server.ts` and the orchestration reactors were the real conflict points; ADR-0004 upstream VCS/checkpoint files added 2026-06-16)
...
| `apps/server/src/vcs/GitVcsDriver.ts` | Core VCS driver (checkpoint create/restore, whole-tree git ops); edited for ADR-0004 to add a thin delegation to fork-local `scopedCheckpointRestore.ts` for the scoped-restore case; legacy whole-tree path untouched | 2 | **High** — upstream-maintained, actively churned; expect rebase conflicts here |
| `apps/server/src/vcs/VcsDriver.ts` | VCS driver interface; edited for ADR-0004 to add optional `paths?` field on the restore input (forwarded verbatim) | 2 | Medium — interface file, changes infrequently but is upstream-owned |
| `apps/server/src/checkpointing/Services/CheckpointStore.ts` | Checkpoint store service interface; edited for ADR-0004 to add optional `paths?` field (forwarded verbatim) | 2 | Medium — interface file, changes infrequently but is upstream-owned |
